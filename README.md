# Solving advanced matrix problems in C to build strong low-level logic for autonomous systems and cybersecurity
---

## 🗺 Matrix Roadmap (50 Exercises)

### 🟢 Level 1: Matrix Fundamentals and Manipulation (Exercises 1-15):
- [x] Reading
- [x] Printing
- [x] Basic Calculations
- [x] Square and Rectangular Matrices

---

### 🟠 Level 2: Advanced Cell and Loop Manipulation (Exercises 16-30): 
- [x] Diagonals
- [x] Triangles
- [x] Swaps
- [x] Flags
- [x] Searches

---

### 🔴 Level 3: Geometric Applications and Cryptography (Exercises 31-43): 
- [x] Matrix Multiplication
- [x] Rotation
- [ ] Shift
- [ ] Coordinate Systems

---

### 🟣 Level 4: Project and Simulation Exercises (Exercises 44-50): 
- [ ] Grid Games
- [ ] Basic Pathfinding

---
